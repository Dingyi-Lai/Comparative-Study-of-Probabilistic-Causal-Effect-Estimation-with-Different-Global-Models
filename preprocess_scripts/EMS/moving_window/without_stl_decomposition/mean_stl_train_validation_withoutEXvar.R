...
## This is the DeepCPNet variant that doesn't consider the alcohol license issuing variable (ALI) as an exogenous variable
...
#df_ALI <- read.csv("license.txt", header = FALSE) # ALI dataset

# Defining output directory, input window size, forecasting horizon, and seasonality respectively.
OUTPUT_DIR = "./datasets/text_data/EMS/moving_window/"
...
for (validation in c(TRUE, FALSE)) {
  for (idr in 1 : nrow(df_train)) {
    print(idr)
    OUTPUT_PATH = paste(OUTPUT_DIR, "ems_", sep = '/')
