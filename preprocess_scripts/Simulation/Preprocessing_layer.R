  ...
  for (amount in amount_of_time_series) {
    for (ln in dgp) {
      for (te in te_intervention){ 
        ...
        # train data
        nam <- paste("Synthetic", length, amount, ln, te, "train", sep = "_")
        ...
        write.csv(data_train, paste0("datasets/text_data/Synthetics/", nam, ".csv"), row.names = FALSE)
        ...
        for(file_type in c("train", "validation", "test")){
            for(i in c(1:amount)){
                ...
                if(file_type=="train"){
                    ...
                    OUTPUT_PATH = paste(OUTPUT_DIR, ser_id, max_forecast_horizon, input_size, sep = '_')
                    sav_df = matrix(
                        NA,
        ...
        nam <- paste("Synthetic", length, amount, ln, te, "test_actual", sep = "_")
        ...
        write.csv(data_test, paste0("datasets/text_data/Synthetics/", nam, ".csv"), row.names = FALSE)
      }
    }
  }
}
