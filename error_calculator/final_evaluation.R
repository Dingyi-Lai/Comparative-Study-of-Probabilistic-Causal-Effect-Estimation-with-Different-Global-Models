...
for(q in quantile_range){
    errors_file_name = args[4] + "_" + str(q)
    ...
    forecasts_file_full_name = paste(root_directory, rnn_forecast_file_path + model_identifier + "_" + str(q) + ".txt", sep = "/")
    ...
    if(q==0.5){
        ...
        mean_MASE = mean(crps_vector)
        median_MASE = median(crps_vector)
        std_MASE = sd(crps_vector)
